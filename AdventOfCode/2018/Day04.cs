    ...
    internal class Day04
    {
        public void Solve()
        {
            ...
            foreach (string line in lines)
            {
                ...
                string type = data[5]; // Guard, falls, wakes
                int minute = int.Parse(data[4]);
                if (type == "Guard")
                {
                    currentGuardId = int.Parse(data[6]);
                    if (!guardsTotalSleepingMinutes.ContainsKey(currentGuardId))
                    {
                        guardsTotalSleepingMinutes.Add(currentGuardId, 0);
                        guardsSleepingMinutes.Add(currentGuardId, new int[60]);
                    }
                }
                if (type == "falls") { sleepStartsAt = minute; }
                if (type == "wakes")
                {
                    for (int i = sleepStartsAt; i < minute; i++) {
                        guardsTotalSleepingMinutes[currentGuardId]++;
                        guardsSleepingMinutes[currentGuardId][i]++;
                    }
                }
            }

            // PART 01
            int guardThatSleepsTheMost = guardsTotalSleepingMinutes.OrderByDescending(kvp => kvp.Value).First().Key;
            int minuteThatIsSleptTheMost = -1, valueOfThatMinute = -1;
            for (int i = 0; i < 60; i++) {
                if (guardsSleepingMinutes[guardThatSleepsTheMost][i] > valueOfThatMinute) {
                    minuteThatIsSleptTheMost = i;
                    valueOfThatMinute = guardsSleepingMinutes[guardThatSleepsTheMost][i];
                }
            }
            Console.WriteLine($"Day04 part1 GuardID:{guardThatSleepsTheMost}, sleeping at total of {guardsTotalSleepingMinutes[guardThatSleepsTheMost]} - minute: { minuteThatIsSleptTheMost}");
            Console.WriteLine($"Day04 part1 answer:{guardThatSleepsTheMost* minuteThatIsSleptTheMost}");

            // PART 02
            int guardIdThatSleepMostOnAMinute = -1, minuteIdOfMostSleep=-1, totalSleepingOnThatMinute = -1;
            foreach (var kvp in guardsSleepingMinutes)
            {
                for (int i = 0; i < kvp.Value.Length; i++)
                {
                    //if (guardsSleepingMinutes[kvp.Key][i] > valueOfThatMinute)
                    if(kvp.Value[i]>totalSleepingOnThatMinute)
                    {
                        guardIdThatSleepMostOnAMinute = kvp.Key;
                        minuteIdOfMostSleep = i;
                        totalSleepingOnThatMinute = kvp.Value[i];
                    }
                }
            }
            Console.WriteLine($"Day04 part2 answer:{guardIdThatSleepMostOnAMinute * minuteIdOfMostSleep}");

        }

        public string input = @"[1518-06-12 23:57] Guard #2633 begins shift
